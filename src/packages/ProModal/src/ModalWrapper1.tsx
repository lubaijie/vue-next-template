...
export default defineComponent({
  ...
  props: {
    ...
    onGetExtHeight: Function as PropType<(height: number) => void>,

    onHeightChange: Function as PropType<(height: string) => void>,
  },
  ...
  setup(props: ModalWrapperProps, { slots, emit }) {
    ...
    useWindowSizeFn<void>(setModalHeight);
    ...
    function listenElResize() {
      ...
      if (!container) return;
      const [start, stop] = useElResize<void>(container, () => {
        setModalHeight();
      });
      stopElResizeFn = stop;
      start();
    ...
    return () => {
      return (
        <div ref={wrapperRef} style={unref(wrapStyle)}>
          <Spin ref={spinRef} spinning={props.loading}  v-slots={{...slots}} />
        </div>
      );
    };
  },
